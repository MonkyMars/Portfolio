...
const Projects = () => {
  ...
  return (
    ...
    >
      ...
      {/* Filter Bar */}
      <div className="flex flex-col md:flex-row gap-4 mb-8 bg-primary-50/50 dark:bg-slate-800/30 p-4 rounded-xl border border-primary-100 dark:border-slate-700/50 shadow-sm">
        ...
        <div className="flex gap-3">
          <Select value={filters.type} onValueChange={(value) => setFilters({...filters, type: value})}>
            <SelectTrigger className="w-[140px] h-10 border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800/80 text-gray-900 dark:text-gray-100">
              <SelectValue placeholder="Project Type" />
            </SelectTrigger>
            <SelectContent className="bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700">
              <SelectItem value="all" className="text-gray-900 dark:text-gray-200 cursor-pointer hover:dark:bg-slate-900/90 hover:bg-gray-100/90">All Types</SelectItem>
              {projectTypes.map((type) => (
                <SelectItem key={type} value={type} className="text-gray-900 dark:text-gray-200 cursor-pointer hover:dark:bg-slate-900/90 hover:bg-gray-100/90">
          ...
          <Select value={filters.tech} onValueChange={(value) => setFilters({...filters, tech: value})}>
            <SelectTrigger className="w-[160px] h-10 border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800/80 text-gray-900 dark:text-gray-100">
              <SelectValue placeholder="Technology" />
            </SelectTrigger>
            <SelectContent className="bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700">
              <SelectItem value="all" className="text-gray-900 dark:text-gray-200 cursor-pointer hover:dark:bg-slate-900/90 hover:bg-gray-100/90">All Technologies</SelectItem>
              {techStacks.map((tech) => (
                <SelectItem key={tech} value={tech} className="text-gray-900 dark:text-gray-200  cursor-pointer hover:dark:bg-slate-900/90 hover:bg-gray-100/90">
                  <Image
                    src={`/icons/${tech.replaceAll('.', '').toLocaleLowerCase()}.png`}
                    alt={tech}
                    width={16}
                    height={16}
                    className={`inline-block mr-2 ${tech.toLocaleLowerCase() === "railway" ? "invert dark:filter-none" : ""}`}
                  />
                  {tech}
                </SelectItem>
              ))}
          ...
          {(filters.search !== "" || filters.type !== "all" || filters.tech !== "all") && (
            <button
              ...
              className="px-4 h-10 text-sm font-medium transition-all rounded-lg bg-primary-100 hover:bg-primary-200 text-primary-600 dark:bg-slate-700/70 dark:hover:bg-slate-700 dark:text-gray-200 border border-primary-200 dark:border-slate-600"
            >
              Clear
            </button>
          )}
